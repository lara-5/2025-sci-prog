...
> **Netflix Case Study:** AI agents generate legacy codebases at lightspeed without guardrails.
...
*   **OpenAI's Codex:** A "Computer Use Agent for Terminal," not just a model.
...
*   **HumanLayer:** If $>40\%$ of context is "noise," reasoning collapses.
...
*   **Anthropic:** Stop building "God Agents." Build **Skills**.
...
*   **Results:** Cognition reduced tool loops by 50%.
...
*   **Arize:** Treat System Prompt as a hyperparameter to optimize.
*   **Workflow:**
...
*   **Pattern:** **Interleaved Thinking** (MiniMax M2).
...
Current LLMs predict syntax tokens. **Meta (FAIR)** argues this is insufficient.
...
*   **METR Observation:** Benchmarks claim AI is superhuman.
...
*   **Cursor (LiveCodeBench):** Evals must update monthly with *new* problems.
...
**VS Code** is designed for human typing (ms), not AI token streams (μs).

*   **DeepMind (Antigravity):**
    *   **Agent Manager:** Surface for async, long-horizon work *above* the editor.
    *   Built by researchers training Gemini.
*   **Amp:** Controls terminal, windowing, render loop — agents "see" system state directly.
